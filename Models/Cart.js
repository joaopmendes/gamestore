...
const cartScheema = new db.Schema({
  items: [{ item: { type: db.Schema.Types.ObjectId, ref: "product" }, quantity: { type: Number, required: true } }],
  totalPrice: { type: Number, required: true },
  owner: { type: db.Schema.Types.ObjectId, ref: "user" },
});
cartScheema.methods.calculateTotal = async function () {
  //* Get Cart instance
...
cartScheema.methods.addProduct = async function (product) {
...
cartScheema.methods.removeProduct = async function (product) {
  //* Get Cart instance
  const me = await this.model("cart").findById(this._id);
...
module.exports = db.model("cart", cartScheema);
